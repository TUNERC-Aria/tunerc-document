# EZ3资料页

## 硬件版本
EZ3当前有两个版本的硬件, 硬件版本可以通过查看飞控上的丝印确定, 以下是硬件的差异:
1.  丝印: F06B-25W35 此版本的电机焊盘较小(普通用户不会收到此版本).
2.  丝印: F06B-25W46 正式销售的第一个版本, 这也是当前购买会得到的版本.

## EZ3 3D模型
我们提供了EZ3的3D文件, 可以供用户在设计机架或生成渲染图时使用.  
文件地址:https://github.com/TUNERC-Aria/tunerc-document/blob/main/docs/Flight-controller/tr-fc-f06b/_FC-F06B.zip

该文件格式为.zip压缩包, 内部包含一个.step文件(文件格式:STEP AP214).  
文件中包含了4个电机连接器和4个避震球, 飞控包装内不含有电机连接器, 避震球是TUNERC推荐的避震球,在飞控的配件包中, 用户需要自行安装.  

## EZ3 测试点信息
以下图片为EZ3上所有测试点的详细信息.